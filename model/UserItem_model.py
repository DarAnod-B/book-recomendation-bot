...
class UserBasedRecommendation:
    ...
    def fit(
        ...
        assert (
            X.shape[0] > BookCount.min_dataset_book.value
        ), f"The dataset must contain at least {BookCount.min_dataset_book.value} ratings from users to create a recommendation."

        # Training the KNN model and creating a pivot table of training data.
        X_pt_grade = (
            X.pivot_table(
                index=self.user_id_col_name,
        ...
        X_pt_grade = X_pt_grade[sorted(X_pt_grade.columns)]
    ...
    def _prepare_y(self, y: pd.DataFrame) -> pd.DataFrame:
        assert (
            y.shape[0] > BookCount.min_user_book.value
        ), f"The user must have at least {BookCount.min_user_book.value} book to make a recommendation."

        # Converts the incoming dataset into a pivot table suitable for working with KNN model.
        y_pt_grade = y.pivot_table(
            index=self.user_id_col_name,
            columns=self.item_id_col_name,
            values=self.grade_col_name,
        )
        ...
        id_items = pd.DataFrame(columns=(self.X_pt_grade.columns))
        y_pt_grade = (
            y_pt_grade.merge(id_items, how='left').fillna(0).astype('int64')
        )
    ...
    def _find_similar_user(self, y_pt_grade: pd.DataFrame) -> pd.DataFrame:
        distances, user_indices_in_knn = self.knn.kneighbors(
            y_pt_grade, n_neighbors=self.n_neighbors
        )
        ...
        data = {
            self.user_id_col_name: user_indices.tolist(),
    ...
    def _recommend(
        self, y_pt_grade: pd.DataFrame, similar_users: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        similar_users_info = similar_users.merge(
            X_stack, on=self.user_id_col_name, how='left'
        ).query(f"{self.grade_col_name} > 0")

        mean_grade_similar_users = (
            similar_users_info.groupby(
                by=self.user_id_col_name, as_index=False
            )[self.grade_col_name]
            .mean()
            .round(2)
        )
        similar_users_info = similar_users_info.merge(
            mean_grade_similar_users,
            on=self.user_id_col_name,
            suffixes=('', '_mean'),
        )
        mean_grade_target_user = (
            y_pt_grade[y_pt_grade.gt(0)].mean(axis=1)[0].round(2)
        )

        sum_weight = similar_users['weight'].abs().sum().round(2)

        item_score = similar_users_info.groupby(self.item_id_col_name).apply(
            self._prediction_calculation,
            mean_grade_target_user=mean_grade_target_user,
            sum_weight=sum_weight,
        )

        item_score = (
            item_score.sort_values(ascending=False)
            .reset_index()
    ...
    def _prediction_calculation(
        self,
        similar_users_info: pd.DataFrame,
        mean_grade_target_user: float,
        sum_weight: float,
    ) -> pd.DataFrame:
        predicted_grade_target_user = (
            mean_grade_target_user
            + (
                (
                    similar_users_info['grade_mean']
                    - similar_users_info[self.grade_col_name]
                )
                * similar_users_info['weight']
            ).sum()
            / sum_weight
        )
